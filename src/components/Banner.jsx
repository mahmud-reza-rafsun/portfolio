...
const Button = ({
    ...
    return <button className={`${baseClasses} ${variants[variant]} ${className}`}>
        {children}
    </button>;
};
const MobileMenu = ({
  ...
  `}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navItems.map(item => <a key={item} href={`#${item.toLowerCase()}`} className="text-gray-700 dark:text-gray-300 hover:text-orange-500 hover:bg-gray-50 dark:hover:bg-gray-800 block px-3 py-2 rounded-md text-base font-medium">
                {item}
            </a>)}
        </div>
...
const Hero = () => <section className="relative z-10 text-center py-16 sm:py-24 px-4">
